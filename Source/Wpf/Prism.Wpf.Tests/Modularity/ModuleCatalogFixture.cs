...
namespace Prism.Wpf.Tests.Modularity
{
    ...
    public class ModuleCatalogFixture
    {
        ...
        public void CanLoadCatalogFromXaml()
        {
            ...
            var catalog = (ModuleCatalog)XamlReader.Load(stream);
        ...
        public void CanAddModule1()
        {
	        ...
	        var moduleInfos = catalog.Modules.OfType<DynamicModuleInfo>();
	        Assert.AreEqual(1, moduleInfos.Count());
            Assert.AreEqual("Module", moduleInfos.First().ModuleName);
            Assert.AreEqual("ModuleType", moduleInfos.First().ModuleType);
            Assert.AreEqual(InitializationMode.OnDemand, moduleInfos.First().InitializationMode);
            Assert.AreEqual(2, moduleInfos.First().DependsOn.Count);
            Assert.AreEqual("DependsOn1", moduleInfos.First().DependsOn[0]);
            Assert.AreEqual("DependsOn2", moduleInfos.First().DependsOn[1]);
        ...
        public void CanAddModule2()
        {
	        ...
	        var moduleInfos = catalog.Modules.OfType<ModuleInfo>();
	        Assert.AreEqual(1, moduleInfos.Count());
            Assert.AreEqual("Module", moduleInfos.First().ModuleName);
            Assert.AreEqual("ModuleType", moduleInfos.First().ModuleType);
            // Assert.AreEqual(InitializationMode.WhenAvailable, moduleInfos.First().InitializationMode);
            Assert.AreEqual(2, moduleInfos.First().DependsOn.Count);
            Assert.AreEqual("DependsOn1", moduleInfos.First().DependsOn[0]);
            Assert.AreEqual("DependsOn2", moduleInfos.First().DependsOn[1]);
        ...
        public void CanAddModule3()
        {
            ...
            Assert.AreEqual(InitializationMode.OnDemand, catalog.Modules.OfType<DynamicModuleInfo>().First().InitializationMode);
        ...
        public void CanAddModule4()
        {
            ...
            Assert.AreEqual("MockModule", catalog.Modules.First().ModuleName);
            Assert.AreEqual(typeof(MockModule).AssemblyQualifiedName, catalog.Modules.First().ModuleType);
            // Assert.AreEqual(InitializationMode.WhenAvailable, catalog.Modules.OfType<ModuleInfo>().First().InitializationMode);
            Assert.AreEqual(2, catalog.Modules.First().DependsOn.Count);
            Assert.AreEqual("DependsOn1", catalog.Modules.First().DependsOn[0]);
            Assert.AreEqual("DependsOn2", catalog.Modules.First().DependsOn[1]);
        ...
        public void CanAddGroup()
        {
	        ...
	        var moduleInfos = catalog.Modules.OfType<DynamicModuleInfo>();
	        Assert.AreEqual(2, moduleInfos.Count());

            var module1 = moduleInfos.First();
            var module2 = moduleInfos.Skip(1).First();
        ...
        private class TestableModuleCatalog : ModuleCatalog
        {
            ...
            public bool LoadCalled => LoadCalledCount > 0;
