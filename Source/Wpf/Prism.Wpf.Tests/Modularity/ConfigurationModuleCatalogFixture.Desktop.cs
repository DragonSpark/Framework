...
namespace Prism.Wpf.Tests.Modularity
{
    ...
    public class ConfigurationModuleCatalogFixture
    {
        ...
        public void ShouldReturnAListOfModuleInfo()
        {
	        ...
	        Assert.AreNotEqual(InitializationMode.WhenAvailable, first.InitializationMode);
            Assert.IsNotNull(first.Ref);
            StringAssert.StartsWith(first.Ref, "file://");
            Assert.IsTrue( first.Ref.Contains(@"MocksModules/MockModuleA.dll"));
        ...
        public void GetStartupLoadedModulesDoesntRetrieveOnDemandLoaded()
        {
            ...
            Assert.AreEqual<int>(0, catalog.Modules.OfType<DynamicModuleInfo>().Count(m => m.InitializationMode != InitializationMode.OnDemand));
        }
