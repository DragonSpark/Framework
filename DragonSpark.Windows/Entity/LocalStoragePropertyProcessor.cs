...
namespace DragonSpark.Windows.Entity
{
	public class LocalStoragePropertyProcessor
	{
		...
		{
			var types = context.GetDeclaredEntityTypes().Select( x => TypeExtensions.GetHierarchy( x, false ).Last() ).Distinct().SelectMany( x => x.Assembly.GetTypes().Where( y => x.Namespace == y.Namespace ) ).Distinct().ToArray();
