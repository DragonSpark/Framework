    ...
    public class UnityRegionNavigationContentLoader : RegionNavigationContentLoader
    {
        ...
        protected override IEnumerable<object> GetCandidatesFromRegion(IRegion region, string candidateNavigationContract)
        {
            ...
            if (!contractCandidates.Any())
            {
                ...
                var matchingRegistration = this.container.Registrations.FirstOrDefault(r => candidateNavigationContract.Equals(r.Name, StringComparison.Ordinal));
                if (matchingRegistration == null)
                {
                    matchingRegistration = this.container.Registrations.FirstOrDefault(r => candidateNavigationContract.Equals(r.RegisteredType.Name, StringComparison.Ordinal));
                }
