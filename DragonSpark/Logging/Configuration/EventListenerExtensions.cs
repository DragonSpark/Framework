...
namespace DragonSpark.Logging.Configuration
{
	...
	{
		public static T ListenTo<T>( this EventListener target, EventLevel level = EventLevel.LogAlways, EventKeywords keywords = Keywords.All ) where T : class
		{
			var result = ListenTo( target, typeof(T), level, keywords );
			return (T)result;
		}

		public static object ListenTo( this EventListener target, Type eventSourceType, EventLevel level, EventKeywords keywords )
		{
