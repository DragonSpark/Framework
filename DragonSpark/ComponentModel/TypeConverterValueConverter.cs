...
namespace DragonSpark.ComponentModel
{
	public class TypeConverterValueConverter : IValueConverter
	{
		...
		{
			return type.IsClass || type.IsInterface || type.IsGenericType && type.GetGenericTypeDefinition().IsAssignableFrom( typeof(Nullable<>) ) ? null : Activator.CreateInstance<object>(type);
		}
		...
		object Convert( object value, Type targetType )
		{
			if ( value != null )
			{
				...
				var resolveFromConverter = ResolveFromConverter( value, targetType, providedType );
				var result = typeConverterType != null ? resolveFromConverter ?? assigned ?? ChangeType( value, targetType )
					             : 
					             assigned ?? resolveFromConverter ?? ChangeType( value, targetType );
				return result;
			}
