...
namespace DragonSpark.Application.Entities.Queries.Runtime.Selection
{
	public class ParameterAwareWhereSelection<TKey, TEntity, T> : IQuery<TKey, T>
	{
		...
		public ParameterAwareWhereSelection(IQueryable<TEntity> queryable, Express<TKey, TEntity> where,
		                                    Func<TKey, IQueryable<TEntity>, IQueryable<T>> selection)
			: this(new Accept<TKey, TEntity>(queryable), where, selection) {}

		public ParameterAwareWhereSelection(IQuery<TKey, TEntity> query, Express<TKey, TEntity> where,
		                                    Func<TKey, IQueryable<TEntity>, IQueryable<T>> selection)
		{
			_query     = query;
			_where     = where;
