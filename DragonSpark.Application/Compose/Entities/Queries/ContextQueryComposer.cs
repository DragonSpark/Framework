...
{
	public sealed class ContextQueryComposer<TIn, TContext, T> where TContext : DbContext
	{
		readonly IContexts<TContext> _contexts;
		readonly IQuery<TIn, T>      _query;

		public ContextQueryComposer(IContexts<TContext> contexts, IQuery<TIn, T> query)
		{
			_contexts = contexts;
			_query    = query;
		}

		public ContextQueryComposer<TIn, TContext, T> Where(Expression<Func<T, bool>> where)
			=> Next(new Where<TIn, T>(_query.Get(), where));

		public ContextQueryComposer<TIn, TContext, T> Where(Expression<Func<TIn, T, bool>> where)
			=> Next(new Where<TIn, T>(_query.Get(), where));

		public ContextQueryComposer<TIn, TContext, T> OrderBy<TProperty>(Expression<Func<TIn, T, TProperty>> property)
			=> Next(new OrderBy<TIn, T, TProperty>(_query.Get(), property));

		public ContextQueryComposer<TIn, TContext, T> OrderBy<TProperty>(Expression<Func<T, TProperty>> property)
			=> Next(new OrderBy<TIn, T, TProperty>(_query.Get(), property));

		public ContextQueryComposer<TIn, TContext, T> OrderByDescending<TProperty>(
		...
		public ContextQueryComposer<TIn, TContext, TOther> OfType<TOther>() where TOther : class, T
			=> Select(x => x.OfType<TOther>());

		public ContextQueryComposer<TIn, TContext, TTo> Select<TTo>(Expression<Func<T, TTo>> select)
			=> Next(new Select<TIn, T, TTo>(_query.Get(), select));

		public ContextQueryComposer<TIn, TContext, TTo> Select<TTo>(Expression<Func<TIn, T, TTo>> select)
			=> Next(new Select<TIn, T, TTo>(_query.Get(), select));

		public ContextQueryComposer<TIn, TContext, TTo>
		...
		public ContextQueryComposer<TIn, TContext, TTo> SelectMany<TTo>(Expression<Func<T, IEnumerable<TTo>>> select)
			=> Next(new SelectMany<TIn, T, TTo>(_query.Get(), select));

		public ContextQueryComposer<TIn, TContext, TTo> SelectMany<TTo>(
		...
		public IntroducedQueryAdapter<TIn, TContext, T, TOther>
			...
			=> new(_contexts, _query, other.Then().Accept<TIn>());
		...
		ContextQueryComposer<TIn, TContext, TTo> Next<TTo>(IQuery<TIn, TTo> next) => new(_contexts, next);
