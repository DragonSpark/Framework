...
sealed class AddHttpClient<T> : IAddClient<T> where T : Options
{
	...
	public IServiceCollection Get(AddHttpClientInput<T> parameter)
	{
		...
		var next       = options.Configure ? _configure(start) : start;
		var configured = configure(next, options);
		if (options.Address is not null)
		{
			_ = configured.ConfigureHttpClient(new EnsureBaseAddress(options.Address).Execute);
		}
