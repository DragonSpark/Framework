...
namespace DragonSpark.Objects.Synchronization
{
	public class ListSynchronizationContext : ObjectResolvingSynchronizationContext
	{
		...
		public override void Synchronize(SynchronizationContainerContext context)
		{
			...
			var type = ListHelper.ResolveElementType( targetList.GetType() );
			...
			ListHelper.Synchronize( sourceList, targetList );
		}
		...
		{
			var sourceType = ListHelper.ResolveElementType( source.GetType() );
		...
		{
			var source = ListHelper.ResolveList( target );
			if ( source != null )
			{
				return source;
			}
