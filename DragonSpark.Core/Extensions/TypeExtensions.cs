...
namespace DragonSpark.Extensions
{
	public static class TypeExtensions
	{
		...
		{
			var result = target.Prepend().Concat( target.GetTypeInfo().ImplementedInterfaces.SelectMany( GetAllInterfaces ) ).Where( x => x.GetTypeInfo().IsInterface ).Distinct().ToArray();
			return result;
		}
