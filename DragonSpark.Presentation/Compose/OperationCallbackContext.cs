...
public sealed class OperationCallbackContext : IResult<EventCallback>
{
	...
	public OperationCallbackContext UpdateActivity(ActivityReceiverInput input)
		=> new(_receiver, new ActivityAwareOperation(_operation, _receiver, input), _activity);
...
public sealed class OperationCallbackContext<T> : IResult<EventCallback<T>>
{
	...
	public OperationCallbackContext(object receiver, IOperation<T> operation, IActivityReceiver? activity = null)
	{
		_receiver  = receiver;
		_operation = operation;
		_activity  = activity;
	...
	public OperationCallbackContext<T> Block(TimeSpan duration)
		=> new(_receiver, new BlockingEntryOperation<T>(_operation, duration), _activity);

	// TODO:
	public OperationCallbackContext<T> UpdateActivityWithRefresh() => UpdateActivity(ActivityReceiverInput.WithRefresh);

	// TODO: Move to UpdateActivity
	public OperationCallbackContext<T> Using(IActivityReceiver receiver) => new(_receiver, _operation, receiver);
	...
	public OperationCallbackContext<T> UpdateActivity(ActivityReceiverInput input)
		=> new(_receiver, new ActivityAwareOperation<T>(_operation, _activity ?? _receiver, input), _activity);

	public EventCallback<T> Get()
		=> EventCallback.Factory.Create(_activity ?? _receiver, new Func<T, Task>(_operation.Allocate));

	public EventCallback Adapt()
		=> EventCallback.Factory.Create(_activity ?? _receiver,
