...
namespace Microsoft.Expression.Interactions.Extensions
{
	...
	{
		private DependencyObject oldObject;
		private DependencyObject newObject;

		public DependencyObject OldObject
		{
			get { return oldObject; }
		}

		public DependencyObject NewObject
		{
			get { return newObject; }
		}

		public NameResolvedEventArgs(DependencyObject oldObject, DependencyObject newObject)
		{
	...
	internal sealed class NameResolver
	{
		...
		private FrameworkElement ActualNameScopeReferenceElement
		{
			get
			{
				if (this.NameScopeReferenceElement == null || !this.NameScopeReferenceElement.IsLoaded())
				{
					return null;
				}
		...
		private void UpdateObjectFromName(DependencyObject oldObject)
		{
			...
			{
				if (!this.NameScopeReferenceElement.IsLoaded())
				{
		...
		{
			Debug.Assert(initialReferenceElement.IsLoaded());
