...
sealed class SameKeys<TFrom, TTo> : ICondition<MigrationInput> where TFrom : class where TTo : class
{
	...
	readonly ISelect<DbContext, ImmutableHashSet<object>> _source, _destination;

	public SameKeys(ISelect<DbContext, ImmutableHashSet<object>> source,
	                ISelect<DbContext, ImmutableHashSet<object>> destination)
	{
		_source      = source;
